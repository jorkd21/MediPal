        ...
        uploadTask.snapshotEvents.listen((TaskSnapshot taskSnapshot) {
          setState(() {
            uploadStatus = taskSnapshot.state == TaskState.running
                ? "${translation(context).upload}(${(taskSnapshot.bytesTransferred / taskSnapshot.totalBytes).toStringAsFixed(1)}%)"
          ...
          if (taskSnapshot.state == TaskState.success) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('${translation(context).uploadOf} ${fileData.name}${translation(context).failed}'),
              ),
            );
          }
        });
